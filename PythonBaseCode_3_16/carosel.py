...
def stateRelay():
    if statePointer == SNAKE_EFFECT:
        ...
        simpleTestEffect()
    elif statePointer == CAROSEL_EFFECT:
        carosel() 
    else:
        idle()
#-------------------STATE----FUNCTIONS-------------------------------
...
def carosel():       
    ...
    #Colours - red may not be ideal, but can change later; just for testing
    if colour_focus == 10:
        colour_focus = 0
    if colour_focus == 0:       #red
        colour = [255, 0, 0]         
    elif colour_focus == 1:     #rose
        colour = [255, 0, 128]
    elif colour_focus == 2:     #magenta
        colour = [255, 0, 255]  
    elif colour_focus == 3:     #violet
        colour = [128, 0, 255]
    elif colour_focus == 4:     #blue
        colour = [0, 0, 255]
    elif colour_focus == 5:     #azure
        colour = [0, 128, 255]
    elif colour_focus == 6:     #cyan
        colour = [0, 255, 255]
    elif colour_focus == 7:     #green
        colour = [0, 255, 0]  
    elif colour_focus == 8:     #yellow
        colour = [255, 255, 0]
    elif colour_focus == 9:     #orange
        colour = [0, 128, 255]
    
    #- 4 columns on and will move clockwise around the cube, changing colour as they rotate
    if substate == 0:   
        for i in range(6):      #
            msg[i] = (level == i)
        if level == 5: 
            massRGBdisplay([0,5,30,35],colour,runs,1)
        elif level == 4:
            massRGBdisplay([0,5,30,35],colour,runs,1)  
        elif level == 3:
            massRGBdisplay([0,5,30,35],colour,runs,1) 
        ...
        for i in range(6):      #
            msg[i] = (level == i)
        if level == 5: 
            massRGBdisplay([4,6,29,31],colour,runs,1)
        elif level == 4:
            massRGBdisplay([4,6,29,31],colour,runs,1)  
        elif level == 3:
            massRGBdisplay([4,6,29,31],colour,runs,1) 
        ...
        for i in range(6):      #
            msg[i] = (level == i)
        if level == 5: 
            massRGBdisplay([3,12,23,32],colour,runs,1)
        elif level == 4:
            massRGBdisplay([3,12,23,32],colour,runs,1)  
        elif level == 3:
            massRGBdisplay([3,12,23,32],colour,runs,1) 
        ...
        for i in range(6):      #
            msg[i] = (level == i)
        if level == 5: 
            massRGBdisplay([2,17,18,33],colour,runs,1)
        elif level == 4:
            massRGBdisplay([2,17,18,33],colour,runs,1)  
        elif level == 3:
            massRGBdisplay([2,17,18,33],colour,runs,1) 
        ...
        for i in range(6):      #
            msg[i] = (level == i)
        if level == 5: 
            massRGBdisplay([1,11,24,34],colour,runs,1)
        elif level == 4:
            massRGBdisplay([1,11,24,34],colour,runs,1)  
        elif level == 3:
            massRGBdisplay([1,11,24,34],colour,runs,1) 
...
def massRGBdisplay(postions, colour, runs, mode = 0):
    #postions is a list of the columns of LEDs that should be on
    #colour, runs and mode same as for RGBdisplay function
    #currently can't do LEDs of different colours in the same phase
    for x in range(36):
        if x in postions:
            RGBdisplay(x, colour, runs, mode)
        else:
            RGBdisplay(x, [0,0,0], runs, mode)
